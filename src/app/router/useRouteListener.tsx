...
const useRouteListener = () => {
    ...
    useLayoutEffect(() => {
        ...
        const handleStateChange = (method: typeof originalPushState, ...args: Parameters<typeof originalPushState>) => {
            const event = new CustomEvent("locationChange");
            method.apply(window.history, args);
            window.dispatchEvent(event);
        };

        window.history.pushState = (...args) => handleStateChange(originalPushState, ...args);
        window.history.replaceState = (...args) => handleStateChange(originalReplaceState, ...args);
